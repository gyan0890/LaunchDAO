...
contract DeployContract {
    ...
    function deploy(string memory name, string memory symbol, string memory description, uint256 totalSupply) external returns (address) {
        ...
        addressContractMap[msg.sender] = address(dao);
    ...
    //Returns the DAO contract address for a particular owner
...
contract DAOLaunch is ERC20 {
    ...
    function getMetadata() external view returns(string memory, string memory, uint256 ) {
        return (daoName, description,maxSupply);
    }
