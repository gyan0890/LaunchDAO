...
export default class DaoCreationService {
  ...
  static async getDaoAddress(cb) {
    ...
    window.contract = await new web3service.eth.Contract(
      deployContractABI,
      parentContract
    );
    const tokenAddress = await window.contract.methods
      .getContract()
  ...
  static async mintToken(tokenContract, cb) {
    ...
    const transactionParameters = {
      ...
      from: window.ethereum.selectedAddress, // must match user's active address.
      data: window.contract.methods.mintToken().encodeABI(),
    };

    try {
      const txHash = web3service.eth
        .sendTransaction(transactionParameters)
        .once("sending", function (payload) {
          console.log(payload);
        })
  ...
  static async getDaoMetadata(tokenContract) {
    ...
    const daoMetadata = await window.contract.methods
      .getMetadata()
      .call({ from: window.ethereum.selectedAddress });
  ...
  static async getAllDaos() {
    ...
    const allDaos = await window.contract.methods
      .getAllContracts()
      .call({ from: window.ethereum.selectedAddress });
